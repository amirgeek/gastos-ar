...
export default function App() {
  ...
  function setAuthFromUser(user) {
    setAuth({ id: user.id, email: user.email, name: user.user_metadata?.name || user.email });
  }
...
function EmptyStateCard({ title, body, cta, type, onAction }) { return <div className="empty-state-card"><span className="eyebrow">Empezá por acá</span><h4>{title}</h4><p>{body}</p>{cta && <button className="submit-btn" type="button" onClick={() => onAction(type)}>{cta}</button>}</div>; }
