...
const OTHER_ACCOUNT_OPTION = { id: 'other', label: 'Otra / personalizada', kind: 'custom', accountType: 'wallet' };
const KNOWN_ACCOUNT_MAP = Object.fromEntries(KNOWN_ACCOUNT_OPTIONS.map((item) => [item.id, item]));
const DEFAULT_YIELD_RATES = KNOWN_ACCOUNT_OPTIONS.filter((item) => Number.isFinite(item.tna)).map((item, index) => ({
  id: `yield-${index + 1}`,
  provider: item.id,
  label: item.label,
  tna: item.tna,
...
function getKnownAccount(provider) { return provider ? KNOWN_ACCOUNT_MAP[provider] || null : null; }
...
export default function App() {
  ...
  const accountsWithYield = useMemo(() => arsAccounts.map((acc) => { const rate = data.yields.rates.find((r) => r.provider === acc.provider); const meta = getKnownAccount(acc.provider); return rate ? { ...acc, tna: rate.tna, label: rate.label, category: rate.category || meta?.yieldType || 'Cuenta remunerada', source: rate.source || meta?.source || 'Carga manual', limit: rate.limit || meta?.limit || null, isMock: typeof rate.isMock === 'boolean' ? rate.isMock : String(rate.source || meta?.source || '').toLowerCase().includes('mock') } : null; }).filter(Boolean), [arsAccounts, data.yields.rates]);
  ...
  const recommendation = useMemo(() => {
    ...
    return { bestYield, highestBalanceAccount, diff, suggestedMove, monthlyGain, monthlyFreeCash };
  }, [bestYield, liquidAccounts, monthlyFreeCash]);
...
function LandingScreen({ onAuth }) {
  ...
  async function submit(e) { e.preventDefault(); setLoading(true); setError(''); try { await onAuth(form, mode); } catch (err) { setError(err.message || 'No se pudo continuar'); } finally { setLoading(false); } }
  function openAuthFlow(nextMode) { setMode(nextMode); setOpenAuth(true); setMobileMenu(false); }
